...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars POLCOIND and POLCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

POLCOIND=${POLCOIND:-${1}/polcoind}
CLI=${POLCOINCLI:-${1}/polcoin-cli}
...
$POLCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$POLCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
