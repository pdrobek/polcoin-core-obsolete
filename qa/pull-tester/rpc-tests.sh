...
export POLCOINCLI=${BUILDDIR}/qa/pull-tester/run-polcoin-cli
export POLCOIND=${REAL_POLCOIND}

#Run the tests

if [ "x${ENABLE_POLCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and polcoind must all be enabled"
fi
